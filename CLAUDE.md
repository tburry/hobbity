...
Headings written as raw HTML in `.astro` files (e.g. `<h2>Magic Items</h2>`) do not get IDs auto-generated—the rehype slug plugin only runs on markdown content. Always add explicit `id` attributes to headings in `.astro` pages so deep links work (e.g. `<h2 id="magic-items">Magic Items</h2>`). Use the same kebab-case slug the markdown plugin would produce.
